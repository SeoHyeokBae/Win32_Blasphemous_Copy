...
namespace sk
{
	...
	bool Sound::LoadWavFile(const std::wstring& path)
	{
		...
		if (FAILED(SoundMgr::GetDevice()->CreateSoundBuffer(&mBufferDesc, &mSoundBuffer, NULL)))
		{
	...
	{
		_mVolume = GetDecibel(volume);
		mSoundBuffer->SetVolume(_mVolume);
	}
